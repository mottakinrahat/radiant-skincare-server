...
const getSingleBuyerFromDB = async (id: string) => {
  ...
  if (!result) throw new ApiError(status.NOT_FOUND, "Buyer not found");
  return result;
};

const updateBuyerDataFromDB = async (id: string, data: { name?: string; contactNumber?: string }) => {
  await getSingleBuyerFromDB(id);
